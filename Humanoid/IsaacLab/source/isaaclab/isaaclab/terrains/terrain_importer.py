...
class TerrainImporter:
    ...
    def import_ground_plane(self, name: str, size: tuple[float, float] = (2.0e6, 2.0e6)):
        ...
        try:
            ...
            # USD文件不存在，创建一个简单平面
            omni.log.warn(
                f"Unable to load ground plane from USD path: {ground_plane_cfg.usd_path}."
                " Creating a simple plane primitive instead."
            )
            ...
            # 创建一个平面原语
            xformPrim = stage.DefinePrim(prim_path, "Xform")
            planePrim = UsdGeom.Plane.Define(stage, f"{prim_path}/plane")
            
            # 设置平面尺寸
            planePrim.CreateSizeAttr().Set(float(max(size)))
            
            # 设置物理属性
            from pxr import PhysxSchema
            
            # 创建刚体API
            rbAPI = PhysxSchema.PhysxRigidBodyAPI.Apply(planePrim.GetPrim())
            rbAPI.CreateDisableGravityAttr().Set(True)
            
            # 创建碰撞API
            collAPI = PhysxSchema.PhysxCollisionAPI.Apply(planePrim.GetPrim())
            
            # 设置材质
            if self.cfg.physics_material is not None:
                materialPath = f"{prim_path}/physicsMaterial"
                self.cfg.physics_material.func(materialPath, self.cfg.physics_material)
                
                # 将材质应用到平面
                from isaaclab.sim.spawners.from_files.materials import bind_physics_material
                bind_physics_material(planePrim.GetPath().pathString, materialPath)
